...
def cargar_ingresos() -> pd.DataFrame:
    ...
    return ing.groupby("fecha", as_index=False)["ingreso_mensual_usd"].sum()
...
def pagina_resumen(df, df_full, fc, serie_col, serie_label):
    ...
    if not ingresos_df.empty:
        ...
        ing_ult = ing_filtrado["ingreso_mensual_usd"].iloc[-1] if not ing_filtrado.empty else None
        ing_prom = ing_filtrado["ingreso_mensual_usd"].mean() if not ing_filtrado.empty else None
    
        c11, c12 = st.columns(2)
        c11.metric("Ingreso estimado último mes", f"${ing_ult:,.0f}" if ing_ult else "—",
                help="Peaje base (ancla pública) + Cargo por Agua Dulce (mecanismo real ACP). Ver docs/METODOLOGIA_PRECIOS.md")
        c12.metric("Ingreso promedio mensual (rango)", f"${ing_prom:,.0f}" if ing_prom else "—")
    
        st.subheader("Ingresos estimados vs. nivel del lago")
        fig_ing = go.Figure()
        fig_ing.add_trace(go.Scatter(x=ing_filtrado["fecha"], y=ing_filtrado["ingreso_mensual_usd"],
                                    mode="lines", name="Ingreso mensual estimado ($)",
                                    line=dict(color="#d62728")))
        fig_ing.update_layout(height=340, margin=dict(t=40, b=10),
                            title="Ingreso mensual estimado (peaje + CAD)",
                            yaxis_title="USD", xaxis_title=None)
        st.plotly_chart(fig_ing, use_container_width=True)
        st.caption("Estimación de orden de magnitud: mecanismo del Cargo por Agua Dulce real, "
                "peaje base sobre ancla pública documentada. Ver docs/METODOLOGIA_PRECIOS.md.")
